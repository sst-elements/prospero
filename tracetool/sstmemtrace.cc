...
KNOB <string> KnobInsRoutine(KNOB_MODE_WRITEONCE, "pintool",
                             "r", "",
                             "Instrument only a specific routine (if not specified all instructions are instrumented");
...
KNOB <string> KnobTraceFormat(KNOB_MODE_WRITEONCE, "pintool",
                              "f", "text",
                              "Output format, \'text\' = Plain text, \'binary\' = Binary, \'compressed\' = zlib compressed");
...
KNOB <UINT32> KnobTraceEnabled(KNOB_MODE_WRITEONCE, "pintool",
                               "d", "1",
                               "Disable until application says that tracing can start, 0=disable until app, 1=start enabled, default=1");
KNOB <UINT64> KnobFileTrip(KNOB_MODE_WRITEONCE, "pintool",
                           "l", "1125899906842624",
                           "Trip into a new trace file at this instruction count, default=1125899906842624 (2**50)");
...
VOID PerformInstrumentCountCheck(THREADID id) {
    ...
    if (thread_instr_id[id].threadInit == 0) {
        std::cout << "PROSPERO: Thread " << id << " starts at instruction "
                  << thread_instr_id[0].insCount << std::endl;
...
VOID RecordMemRead(VOID *addr, UINT32 size, THREADID thr) {
    ...
    if (0 == trace_format) {
        ...
        copy(RECORD_BUFFER, &size, sizeof(uint64_t) + sizeof(char) + sizeof(uint64_t),
             sizeof(uint32_t));

        if (1 == trace_format) {
            if (thr < max_thread_count && (traceEnabled > 0)) {
                fwrite(RECORD_BUFFER,
                       sizeof(uint64_t) + sizeof(uint64_t) + sizeof(uint32_t) + sizeof(char), 1,
                       trace[thr]);
                thread_instr_id[thr].readCount++;
            }
        } else {
...
VOID InstrumentSpecificRoutine(RTN rtn, VOID *v) {
    if (RTN_Name(rtn) == "prospero_enable_tracing") {
        ...
        std::cout << "PROSPERO: Found routine: " << RTN_Name(rtn)
                  << ", instrumenting for tracing..." << std::endl;
...
VOID Fini(INT32 code, VOID *v) {
    ...
    std::cout << "PROSPERO: Main thread exists with " << thread_instr_id[0].insCount
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "PROSPERO: User requests that a maximum of " << max_thread_count
              << " threads are instrumented" << std::endl;
    std::cout << "PROSPERO: File buffer per thread is " << KnobFileBufferSize.Value() << " bytes"
              << std::endl;
    ...
    if (KnobTraceFormat.Value() == "text") {
        ...
        for (UINT32 i = 0; i < max_thread_count; ++i) {
            sprintf(nameBuffer, "%s-%lu-0-bin.trace", KnobTraceFile.Value().c_str(),
                    (unsigned long) i);
