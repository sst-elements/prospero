...
ProsperoCompressedBinaryTraceReader::ProsperoCompressedBinaryTraceReader(Component *owner,
                                                                         Params &params) :
...
ProsperoCompressedBinaryTraceReader::ProsperoCompressedBinaryTraceReader(ComponentId_t id,
                                                                         Params &params,
                                                                         Output *out) :
    ...
    if (Z_NULL == traceInput) {
        output->fatal(CALL_INFO, -1,
                      "%s, Fatal: attempted to open: %s but zlib returns error condition.\n",
...
ProsperoTraceEntry *ProsperoCompressedBinaryTraceReader::readNextEntry() {
    ...
    if (bytesRead == recordLength) {
        ...
        output->verbose(CALL_INFO, 2, 0,
                        "Did not read a full record from the compressed trace, returning empty request.\n");
