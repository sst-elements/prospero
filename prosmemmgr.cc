...
uint64_t ProsperoMemoryManager::translate(const uint64_t virtAddr) {
    ...
    output->verbose(CALL_INFO, 2, 0, "Translating virtual address %"
    PRIu64
    ", page offset=%"
    PRIu64
    ", start virt=%"
    ...
    if (findEntry == pageTable.end()) {
        output->verbose(CALL_INFO, 2, 0, "Translation requires new page, creating at physical: %"
        PRIu64
        "\n", nextPageStart);
    ...
    output->verbose(CALL_INFO, 2, 0, "Translated physical page to %"
    PRIu64
    " + offset %"
    PRIu64
    " = final physical %"
