...
ProsperoComponent::ProsperoComponent(ComponentId_t id, Params &params) :
    ...
    if (!reader) {
        std::string traceModule = params.find<std::string>("reader",
        ...
        reader = loadAnonymousSubComponent<ProsperoTraceReader>(traceModule, "reader", 0,
    ...
    output->verbose(CALL_INFO, 1, 0, "Configured Prospero page size for %"
    PRIu64
    " bytes.\n", pageSize);

    cacheLineSize = (uint64_t) params.find<uint64_t>("cache_line_size", 64);
    output->verbose(CALL_INFO, 1, 0, "Configured Prospero cache line size for %"
    PRIu64
    " bytes.\n", cacheLineSize);

    std::string prosClock = params.find<std::string>("clock", "2GHz");
    // Register the clock
    TimeConverter *time = registerClock(prosClock, new Clock::Handler<ProsperoComponent>(this,
                                                                                         &ProsperoComponent::tick));
    ...
    output->verbose(CALL_INFO, 1, 0, "Configured maximum outstanding transactions for %"
    PRIu32
    "\n", maxOutstanding);

    maxIssuePerCycle = (uint32_t) params.find<uint32_t>("max_issue_per_cycle", 2);
    output->verbose(CALL_INFO, 1, 0, "Configured maximum transaction issue per cycle %"
    PRIu32
    "\n", maxIssuePerCycle);
    ...
    cache_link = loadUserSubComponent<Interfaces::SimpleMem>("memory", ComponentInfo::SHARE_NONE,
                                                             time,
                                                             new SimpleMem::Handler<ProsperoComponent>(
                                                                 this,
                                                                 &ProsperoComponent::handleResponse));
    if (!cache_link) {
        Params par;
        par.insert("port", "cache_link");
        cache_link = loadAnonymousSubComponent<Interfaces::SimpleMem>("memHierarchy.memInterface",
                                                                      "memory", 0,
                                                                      ComponentInfo::INSERT_STATS |
    ...
    output->verbose(CALL_INFO, 1, 0, "Creating memory manager with page size %"
    PRIu64
    "...\n", pageSize);
...
void ProsperoComponent::finish() {
    ...
    output->output("------------------------------------------------------------------------\n");
    output->output("- Completed at:                          %"
    PRIu64
    " ns\n", nanoSeconds);
    output->output("- Cycles with ops issued:                %"
    PRIu64
    " cycles\n", cyclesWithIssue);
    output->output("- Cycles with no ops issued (LS full):   %"
    PRIu64
    " cycles\n", cyclesWithNoIssue);

    output->output("------------------------------------------------------------------------\n");
    output->output("- Reads issued:                          %"
    PRIu64
    "\n", readsIssued);
    output->output("- Writes issued:                         %"
    PRIu64
    "\n", writesIssued);
    output->output("- Split reads issued:                    %"
    PRIu64
    "\n", splitReadsIssued);
    output->output("- Split writes issued:                   %"
    PRIu64
    "\n", splitWritesIssued);
    output->output("- Bytes read:                            %"
    PRIu64
    "\n", totalBytesRead);
    output->output("- Bytes written:                         %"
    PRIu64
    "\n", totalBytesWritten);
    ...
    sprintf(buffBWCombined, "%f B/s",
            ((double) (totalBytesReadDbl + totalBytesWrittenDbl) / secondsDbl));
...
bool ProsperoComponent::tick(SST::Cycle_t currentCycle) {
    if (NULL == currentEntry) {
        output->verbose(CALL_INFO, 16, 0, "Prospero execute on cycle %"
        PRIu64
        ", current entry is NULL, outstanding=%"
        PRIu32
        ", maxOut=%"
        PRIu32
        "\n",
    ...
    for (uint32_t i = 0; i < maxIssuePerCycle; ++i) {
        if (currentCycle >= currentEntry->getIssueAtCycle()) {
            ...
            output->verbose(CALL_INFO, 8, 0, "Not issuing on cycle %"
            PRIu64
            ", waiting for cycle: %"
            PRIu64
            "\n",
...
void ProsperoComponent::issueRequest(const ProsperoTraceEntry *entry) {
    ...
    if (lineOffset + entryLength > cacheLineSize) {
        ...
        if (lowerLength + upperLength != entryLength) {
            output->fatal(CALL_INFO, -1, "Error: split cache line, lower size=%"
            PRIu64
            ", upper size=%"
            PRIu64
            " != request length: %"
        ...
        const uint64_t upperAddress = memMgr->translate(
            (lowerAddress - (lowerAddress % cacheLineSize)) + cacheLineSize);
        ...
        reqUpper->setVirtualAddress(
            (entryAddress - (entryAddress % cacheLineSize)) + cacheLineSize);
